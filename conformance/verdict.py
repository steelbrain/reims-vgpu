...
because the first two say a reading is *not* about this device:
  ...
  guest FAIL, known    a defect already written down. Quiet.
  guest FAIL, new      a regression, and the reason this tool exits non-zero.
  guest PASS, known    the expectation is stale. Also non-zero: an expectations
                       file nobody prunes stops being a list of defects and
                       becomes a list of cases nobody looks at.
...
def main():
    ...
    ap.add_argument("--expect", help="expectations/known-failures.txt")
    ...
    known = expectations(args.expect)
    ...
    for name in sorted(set(native) | set(guest)):
        ...
        if n and n[0] == "FAIL":
        ...
        elif g[0] == "FAIL" and name in known:
            verdict, note = "known", known[name] or detail
        elif g[0] == "FAIL":
            verdict, note = "REGRESSION", detail
        elif name in known:
            verdict, note = "FIXED", "passes now; drop it from the expectations file"
            ...
            verdict, note = "ok", ""
        if verdict.isupper() or verdict in ("known", "not-covered"):
            bad += verdict.isupper()
    ...
    print(f"known failures {len(known)}, unexplained {bad}")
